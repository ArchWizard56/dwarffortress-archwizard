...
pkgname=dwarffortress-archwizard
pkgver=0.47.04
_pkgver=47_04
pkgrel=1
pkgdesc="A single-player fantasy game in which you build a dwarven outpost or play an adventurer in a randomly generated world, with archwizard's changes"
arch=(x86_64)
...
package() {
  ...
  cp -rf "$srcdir"/CLA-0.47.xx-v26.3/* "$pkgdir"/opt/$pkgname

  #Give girl dwarves beards
  sed -i 's/BODY_DETAIL_PLAN:FACIAL_HAIR_TISSUE_LAYERS/[BODY_DETAIL_PLAN:FACIAL_HAIR_TISSUE_LAYERS]/g' "$pkgdir"/opt/$pkgname/raw/objects/creature_standard.txt
}
